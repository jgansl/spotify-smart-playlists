...
const login_redirect_route = process.env.REDIRECT_URI_SPOTIFY;
...
visY = [
  1, 6, 4, 0, -4, -6, -1, -6, -6.5, -5, -3.5, -4, 0, 4, 3.5, 5, 6.5, 6, 1,
];
    ...
    for (let i = 1; i < tracks.length; i++) {
      for (let j = 0; j < tracks.length; j++) {
        if (tracks[j]["used"] == false) {
          distance = Math.abs(
            song["analysis"]["danceability"] -
              tracks[j]["analysis"]["danceability"]
          );
          distance =
            distance +
            Math.abs(
              song["analysis"]["energy"] - tracks[j]["analysis"]["energy"]
            );
          distance =
            distance +
            Math.abs(
              song["analysis"]["loudness"] - tracks[j]["analysis"]["loudness"]
            ) /
              10; //scale to ~[0-1]
          distance =
            distance +
            Math.abs(
              song["analysis"]["tempo"] - tracks[j]["analysis"]["tempo"]
            ) /
              100; //scale to ~[0-1]
          distance =
            distance +
            Math.abs(song["analysis"]["key"] - tracks[j]["analysis"]["key"]) /
              5; //scale to ~[0-1]
...
function createNewPlaylist() {
  ...
  if (createdPlaylistName == playlistName + ".optimal") {
    ...
    let newPlaylistDescription =
      playlistDescription + " ~ Optimised by Smart Shuffle.";
    ...
    fetch(API_ENDPOINT + NEW_PLAYLIST_ENDPOINT_ID, fetchOptions)
...
function addTracks(reorderedURIs, createdPlaylistID) {
  ...
  const fetchOptions = {
    ...
    body: JSON.stringify({
      uris: reorderedURIs.slice(0, MAX_AUDIO_ANALYSIS_LENGTH),
    }),
  };

  fetch(API_ENDPOINT + TRACKS_ENDPOINT_ID, fetchOptions)
    .then(function (response) {
      return response.json();
    })
    .then(function (json) {
      // console.log(json);
      if (reorderedURIs.length > MAX_AUDIO_ANALYSIS_LENGTH) {
        //Recusive addition of max number of songs
        addTracks(
          reorderedURIs.slice(MAX_AUDIO_ANALYSIS_LENGTH, reorderedURIs.length),
          createdPlaylistID
        );
      }
    })
...
function fetchTrackAnalysis(trackIDs, startIndex = 0) {
  ...
  ANALYSIS_ENDPOINT_ID = AUDIOANALYSIS_ENDPOINT(
    trackIDs.slice(startIndex, startIndex + MAX_AUDIO_ANALYSIS_LENGTH)
  );
  ...
  fetch(API_ENDPOINT + ANALYSIS_ENDPOINT_ID, fetchOptions)
    .then(function (response) {
      return response.json();
    })
    .then(function (json) {
  ...
  fetch(ENDPOINT, fetchOptions)
    ...
    .then(function (json) {
      ...
      document.getElementById("warning").innerHTML =
        "⚠️ This will create a new playlist named " +
        playlists[button_id]["name"] +
        ".optimal";
      // for (let pageIndex = 0; pageIndex < trackIDs.length; pageIndex += MAX_AUDIO_ANALYSIS_LENGTH){
      ...
      if (json["next"] != null) {
        ...
        for (
          let pageIndex = 0;
          pageIndex < trackIDs.length;
          pageIndex += MAX_AUDIO_ANALYSIS_LENGTH
        ) {
          fetchTrackAnalysis(trackIDs, (startIndex = pageIndex)); //Call for analysis
        }
      }
...
function fetchProfileInformation() {
  ...
  fetch(API_ENDPOINT + USER_ENDPOINT, fetchOptions)
    .then(function (response) {
      return response.json();
    })
    .then(function (json) {
